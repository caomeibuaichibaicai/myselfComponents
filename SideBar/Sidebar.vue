<template>
  ...
  <ul id="Sidebar" v-bind:class='["first-nav","noselect",{"iscollapse":sidebar.isCollapse}]'>
    ...
    <li v-for="(firstitem,index) in sideBarList"
        :index="firstitem[menuIndex]"
        :key="firstitem[menuIndex]"
        v-bind:class='["first-nav-items",{"first-nav-active":sidebar.secState&&firstitem[menuIndex]-1===sidebar.firstActiveNow}]'
    >
      ...
      <ul class="second-nav">
        ...
        <li v-bind:class='["second-nav-items",{"second-nav-active":sidebar.secState&&seconditem[menuIndex]-1===sidebar.secondActiveNow}]'
        ...
        >
          <a class="nav-link oneline-ellipsis" v-text="seconditem[menuLabel]"
             @click="linkTo('/'+firstitem[menuRouter]+'/'+seconditem[menuRouter],seconditem[menuIndex]-1)"></a>
        </li>
      </ul>
    </li>
  </ul>
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        ...
        cinfo && !ns.base.isEmpty(cinfo) ? store.sideBar.open() : store.sideBar.close();
      },
      /**
      ...
      firstnavclick(index) {
        ...
        if (index === this.sidebar.firstActiveNow) {
          ...
          store.sideBar.setFirstActive(index);
      ...
      toggleSecActiveShow() {
        ...
        if (this.activeStore.first === this.sidebar.firstActiveNow) {
          store.sideBar.setSecondActive(this.activeStore.second);
        }
        else {
          /*
          ...
          */
          store.sideBar.setSecondActive(null);
        }
      },
      ...
      linkTo(url, index) {
        ...
        store.sideBar.setSecondActive(index);
...
<style rel="stylesheet/scss" lang="scss">
  @import "../../../assets/css/Modular/sidebar/sidebar";
</style>
