<template>
	<el-form :inline="true" ref="ownerSelectForm" :model="ownerSelectForm" id="ownerSelect" class="invariable ownerSelect el-group-top-line" :rules="rules" label-width="150px">
		...
		<el-form-item label="证件号码" prop="certificate" style="float:left">
			<el-input class="width200" readonly disabled v-model="ownerSelectForm.certificate" size="small"></el-input>
		</el-form-item>
		<!--移动电话-->
		<el-form-item :label="phoneNumTit" prop="phoneNum" :label-width="secLabelWidth" style="float: left">
...
<script>
	...
	export default {
		...
		methods: {
			...
			//更新业主选择返回信息
			updateReturnData() {
				this.returnData.newRentOwner = this.newRentOwner;
			...
			assignBindData() {
				if(this.bindData.fromModelData.newRentOwner) {
					this.ownerSelectForm.ownerName = this.bindData.fromModelData.newRentOwner.ownerName;
					...
					this.ownerAssign(this.bindData.fromModelData.newRentOwner)
					this.updateReturnData();
					this.$emit('update-returnData', this.returnData)
				}
				//产权人赋值
