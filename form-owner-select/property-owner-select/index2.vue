<template>
	<el-form :inline="true" ref="propertySelectForm2" :model="propertySelectForm2" id="propertySelectForm2" class="invariable propertySelectForm2 el-group-top-line" :rules="rules" label-width="150px">
		<el-form-item label="产权人3" prop="ownerName">
			<el-select v-model="propertySelectForm2.ownerName" filterable remote
				reserve-keyword
				placeholder="请输入关键词"
		...
		<el-form-item label="与业主关系" prop="ownerRelationship" class="width100">
			<el-select v-model="propertySelectForm2.ownerRelationship" size="small" @change="ownerRelationChange" placeholder="请选择关系">
		...
		<el-form-item label="证件号码" style="float:left">
			<el-input class="width200" disabled readonly v-model="propertySelectForm2.certificate" placeholder="请输入内容" size="small"></el-input>
		</el-form-item>
		<!--移动电话-->
		<el-form-item :label="phoneNumTit" style="float:left">
			<el-input class="width200" disabled readonly v-model="propertySelectForm2.phoneNum" placeholder="请输入内容" size="small"></el-input>
		</el-form-item>
	...
	export default {
		...
		methods: {
			...
			ownerRelationChange(val) {
				this.propertySelectForm2.ownerRelationship = val;
			...
			assignPropertyData(val){
				...
				this.propertySelectForm2.certificate = val.certificate;
				if(this.ownerType == 0) {
					this.phoneNumTit = '移动电话'
					this.propertySelectForm2.phoneNum = this.mobile;
				} else {
					this.phoneNumTit = '联系电话'
					this.propertySelectForm2.phoneNum = this.linkmanPhone
				}
			...
			handleSelect(item) {
				for(let i = 0, val; val = this.list[i]; i++) {
					if(val.value === item) {
						...
						this.linkmanPhone = '';
						this.ownerType = 1;
						this.propertySelectForm2.certificate = '';
						this.propertySelectForm2.phoneNum = "";
						this.newProperty = {};
					}
				};
				this.$emit('update-returnData', this.newProperty)
			...
			listenValidate() {
				let $vali = false;
				this.$refs['propertySelectForm2'].validate((valid) => {
					$vali = valid
				})
				return $vali
			},
			lsitResetForm() {
				EventHub.$on('invariableFormReset', () => {
					this.$refs['propertySelectForm2'].resetFields();
				})
			},
			//新增完客户直接赋值
			afterAddSuccess(params){
				this.remoteMethod(params.formData.modelData.ownerName,(val)=>{
					...
					this.propertySelectForm2.ownerRelationship = '';
					this.propertySelectForm2.ownerName = val.ownerName;
			...
			assignData(){
				if(this.propertyBind){
					...
					this.propertySelectForm2.ownerRelationship = this.propertyBind.ownerRelationship;
					this.propertySelectForm2.ownerName = this.propertyBind.ownerName;
