<template>
	<el-form :inline="true" ref="propertySelectForm3" :model="propertySelectForm3" id="propertySelectForm3" class="invariable propertySelectForm3 el-group-top-line" :rules="rules" label-width="150px">
		<el-form-item label="产权人4" prop="ownerName">
			<el-select v-model="propertySelectForm3.ownerName" filterable remote
				reserve-keyword
				placeholder="请输入关键词"
		...
		<el-form-item label="与业主关系" prop="ownerRelationship" class="width100">
			<el-select v-model="propertySelectForm3.ownerRelationship" size="small" @change="ownerRelationChange" placeholder="请选择关系">
		...
		<el-form-item label="证件号码" style="float:left">
			<el-input class="width200" disabled readonly v-model="propertySelectForm3.certificate" placeholder="请输入内容" size="small"></el-input>
		</el-form-item>
		<!--移动电话-->
		<el-form-item :label="phoneNumTit" style="float:left">
			<el-input class="width200" disabled readonly v-model="propertySelectForm3.phoneNum" placeholder="请输入内容" size="small"></el-input>
		</el-form-item>
	...
	export default {
		...
		methods: {
			...
			ownerRelationChange(val) {
				this.propertySelectForm3.ownerRelationship = val;
			...
			assignPropertyData(val){
				...
				this.propertySelectForm3.certificate = val.certificate;
				if(this.ownerType == 0) {
					this.phoneNumTit = '移动电话'
					this.propertySelectForm3.phoneNum = this.mobile;
				} else {
					this.phoneNumTit = '联系电话'
					this.propertySelectForm3.phoneNum = this.linkmanPhone
				}
			...
			handleSelect(item) {
				for(let i = 0, val; val = this.list[i]; i++) {
					if(val.value === item) {
						...
						this.linkmanPhone = '';
						this.ownerType = 1;
						this.propertySelectForm3.certificate = '';
						this.propertySelectForm3.phoneNum = "";
						this.newProperty = {};
					}
				};
				this.$emit('update-returnData', this.newProperty)
			...
			listenValidate() {
				let $vali = false;
				this.$refs['propertySelectForm3'].validate((valid) => {
					$vali = valid
				})
				return $vali
			},
			lsitResetForm() {
				EventHub.$on('invariableFormReset', () => {
					this.$refs['propertySelectForm3'].resetFields();
				})
			},
			//新增完客户直接赋值
			afterAddSuccess(params){
				this.remoteMethod(params.formData.modelData.ownerName,(val)=>{
					...
					this.propertySelectForm3.ownerRelationship = '';
					this.propertySelectForm3.ownerName = val.ownerName;
			...
			assignData(){
				if(this.propertyBind){
					...
					this.propertySelectForm3.ownerRelationship = this.propertyBind.ownerRelationship;
					this.propertySelectForm3.ownerName = this.propertyBind.ownerName;
